...
> **Инверсия управления (IoC)** — главный принцип, отличающий фреймворк от библиотеки. При использовании библиотеки _ты_ вызываешь её код. При использовании фреймворка _он_ вызывает твой код. Это называется **"принцип Голливуда"**: _"Не звони нам — мы сами тебе позвоним"_ (Don't call us, we'll call you).
...
3. **Архитектурные решения:**
    ...
    - **Библиотека:** Не накладывает ограничений на архитектуру приложения, позволяя использовать ее функции по мере
      необходимости.
4. **Степень "жёсткости" (Opinionated vs Non-opinionated):**
    - **Opinionated** (Spring Boot, Ruby on Rails) — диктуют структуру, соглашения по именованию, способы конфигурации.
      Меньше гибкости, но быстрее старт.
    - **Non-opinionated** (Spring Framework без Boot) — предоставляют инструменты, но оставляют архитектурные решения
      разработчику.
5. **Расширяемость и конфигурация:**
...
- **Разрабатывают профессионалы.** Считается, что фреймворки, тем более известные и промышленные пишут профессионалы
  своего дела, которые знают что к чему.

_Обратите внимание, что в современном мире зачастую ищут уже не просто программистов, знающих ЯП, а программистов
знающих фреймворк(и) на каком-то ЯП._
