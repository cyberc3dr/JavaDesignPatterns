...
Альтернативный способ разрешения конфликта при наличии нескольких реализаций одного интерфейса — аннотация ```@Primary```.
Если пометить один из бинов аннотацией ```@Primary```, то при использовании ```@Autowired``` без ```@Qualifier```
будет внедрён именно этот бин.
...
class Person {
    @Autowired
    private Pet pet; // будет внедрён Cat, т.к. он помечен @Primary
}
```

Разница с ```@Qualifier```:
- ```@Primary``` задаёт бин по умолчанию на стороне **провайдера** (на самом бине).
- ```@Qualifier``` явно указывает конкретный бин на стороне **потребителя** (в месте внедрения).
- Если указан и ```@Primary```, и ```@Qualifier```, приоритет имеет ```@Qualifier```.

<h5 align="center">
    Рекомендация Spring по выбору типа DI
</h5>

Spring рекомендует использовать **constructor injection** как предпочтительный способ внедрения зависимостей:

1. **Неизменяемость (immutability)** — поля можно объявить ```final```, что гарантирует их неизменность после создания объекта.
2. **Не-null гарантии** — объект не может быть создан без всех обязательных зависимостей.
3. **Полная инициализация** — компонент возвращается клиентскому коду полностью инициализированным.

Setter injection рекомендуется для **опциональных** зависимостей, которые могут иметь значения по умолчанию.
Field injection не рекомендуется, так как затрудняет тестирование и скрывает зависимости класса.