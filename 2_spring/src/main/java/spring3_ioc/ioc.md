...
абстрактное, не видимое нам, хранилище, где Spring содержит подвластные ему объекты.
...
*Упрощенно: под контекстом подразумевается то, откуда получает данные **Spring Container***.
...
и ```resources``` (с иконочкой БД). В ```java``` пишется код проекта, а в ```resources``` хранятся все конфигурационные
файлы. Это типичная структура для Java проектов. Во всех фреймворках/библиотеках определены пути по умолчанию именно к
...
2. Через сеттеры.
3. Через внутренний фабричный метод (метод определённый внутри класса создаваемых объектов).
...
Задание значений в конструкторе/сеттерах происходит при помощи аннотации ```@Value```, где можно задать явное значение
или воспользоваться подстановкой из файла ```resources/*.property```.
...
При конфигурации через Java код доступно два способа инициализации бинов: через фабрику бинов или через сканирование
пакета на наличие ```@Component``` классов.
...
3. Внутри ```@Configuration``` класса создаются методы получения бинов (Фабричные методы бинов) и помечаются
   аннотацией ```@Bean```. Сигнатура метода должна выглядеть следующим образом: ```public TypeA typeA()```, а внутри
...
С одной стороны такой подход дает больший контроль над созданием, внедрением зависимостей. Всё видно, всё на ладони.
