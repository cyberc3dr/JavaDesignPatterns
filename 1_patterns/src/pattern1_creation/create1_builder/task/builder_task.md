...
1. Создать класс ```Burger``` со следующими свойствами:
...
2. **Реализовать паттерн Билдер** для класса ```Burger``` - ```BurgerBuilder``` со следующими свойствами:
    ...
    - Реализует **Fluent Interface** - методы билдера должны возвращать тип самого билдера (`BurgerBuidler`),
      `return this`.
...
`enum` лучше делать отдельными классами в отдельном файле как и обычные классы.
...
_Не забудьте импортировать необходимые классы._
