...
# [Пример](code/WithoutAny.java) без какой-либо синхронизации
...
# [Пример](code/JoinMain.java) с синхронизацией потоков

Поскольку нам необходимо получить результат вычислений всех потоков и только потом вывести результат, то необходимо
чтобы main поток дождался выполнения побочных потоков. Достичь этого можно при помощи метода `join()`.
...
# [Пример](code/VolatileMain.java) с volatile переменной
...
# [Пример](code/AtomicMain.java) с atomic переменной

При использовании ```AtomicInteger``` в данной задаче мы получим ожидаемый результат.

Получаем ожидаемый результат ```1_000_000```. Операции с Atomic переменными гарантированно выполняются в отведенный квант
...
# [Пример](code/MutexCorrectMain.java) с Mutex (правильный)
...
# [Пример](code/MutexIncorrectMain.java) с Mutex (не правильный)
...
# [Пример](code/SynchronizationMain.java) с synchronized
