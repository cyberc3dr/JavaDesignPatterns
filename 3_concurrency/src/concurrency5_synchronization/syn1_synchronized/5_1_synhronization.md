...
[CounterExample.java](code/CounterExample.java) — 10 потоков инкрементируют счётчик через `synchronized`-методы. Демонстрирует корректный результат 1 000 000 и необходимость синхронизировать и запись, и чтение.
...
[LoggerExample.java](code/LoggerExample.java) — 5 потоков параллельно пишут в лог через `static synchronized`-метод. Строки печатаются без перемешивания.
...
[BankAccountExample.java](code/BankAccountExample.java) — конкурентные пополнения и списания через `synchronized(lock)` с приватным объектом-монитором.
