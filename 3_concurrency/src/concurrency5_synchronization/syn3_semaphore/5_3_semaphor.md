...
## [Пример](code/ResourceAccess.java): семафор для ограничения доступа к ресурсу

Предположим есть ресурс к которому не более 3 потоков могут обращаться одновременно. 10 потоков конкурируют за доступ — семафор с 3 разрешениями пропускает не более 3 одновременно.
